                ...
                @if(!empty($userCarts) and !$userCarts->isEmpty())
                                    ...
                                    <div class="navbar-cart-info">
                                        ...
                                        <div class="price mt-10">
                                            @if($cart->webinar->getDiscount($cart->ticket) > 0)
                                                <span class="text-primary font-weight-bold">{{ $currency }}{{ number_format($cart->webinar->price - $cart->webinar->getDiscount($cart->ticket), 2, ".", "") + 0 }}</span>
                                                <span class="off ml-15">{{ $currency }}{{ number_format($cart->webinar->price, 2, ".", "") + 0 }}</span>
                                            @else
                                                <span class="text-primary font-weight-bold">{{ $currency }}{{ number_format($cart->webinar->price, 2, ".", "") + 0 }}</span>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="navbar-cart-actions">
                        <div class="navbar-cart-total mt-15 border-top d-flex align-items-center justify-content-between">
                            ...
                            <strong class="text-primary font-weight-bold">{{ $currency }}{{ !empty($totalCartsPrice) ? number_format($totalCartsPrice, 2, ".", "") + 0 : 0 }}</strong>
                        </div>
